...
contract NonBestRateQuery {
    ...
    returns (uint256) // amountOut 
    {
        ...
        // uint256 _amountOut = non.getDestinationReturnAmount(routes[i], src, dest, srcAmount, PARTNER_INDEX);
        bytes memory payload = abi.encodeWithSelector(non.getDestinationReturnAmount.selector, route, src, dest, srcAmount, PARTNER_INDEX);
        (bool success, bytes memory data) = address(non).staticcall(payload);
        if (success) {
